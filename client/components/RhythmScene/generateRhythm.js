...
function createRhythms(timeSigString, rhythmTypeString) {
  ...
  while (Date.now() < now + timeout) {
    ...
    const curRhythm = randomRhythmMeasure(noteValues, timeSigValue);
...
function randomRhythmMeasure(noteValues, timeSigValue) {
  /*
  ...
  */
  ...
  while (timeSigValue - measureCalculation > 0) {
    ...
    if (timeSigValue === (3 / 4) && randomNoteStr === '3/4') {
      continue;
    }
    if (timeSigValue === (3 / 4) && randomNoteStr === '1/2') {
      continue;
    }
...
function getRhythmDisplay(rhythmArray, rhythmTypeSelection, timeSigString) {
  ...
  return `${rhythmArray.length} Measures of Random Rhythms - ${rhythmTypeSelection} in ${timeSigString}
  Guide to rhythmic values: 1/2 = half note. 1/4 = quarter note. 3/4 = dotted half note. 1/16 = sixteenth note. 3/16 = dotted 8th note. 1/6 = quarter note triplet (2 tied 8th note triplets). 1/12 = 8th note triplet. 1/24 = 16th note triplet. 5/24 = 8th note tied to an 8th note triplet.
  ${rhythmArray.join('\n')}`;
}
